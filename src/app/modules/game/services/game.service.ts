...
export class GameService {
  ...
  private move(
    ...
    if (this.theEnd || !this.canIMove(dimX, false, reverse)) { return; }
  ...
  private generateItems(length = 2): void {
  ...
  private canIMove(dimX: 'row' | 'col', skipDir = true, forward = false): boolean {
  ...
  private generateAvailableCells(): void {
    for (let row = 1; row <= this.size; row++) { // 4
      for (let col = 1; col <= this.size; col++) { // 4 x 4
        // 14: 1 = row, 4 = col
